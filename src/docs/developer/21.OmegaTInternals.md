...
Editor marker system as a map — which `IMarker` runs for the whole document,
which only for the active segment, which only while the source side is shown:

![OmegaT editor marker landscape](assets/omegat-marker-landscape-map.svg)
...
See [Plugins and Manifest](19.PluginManifest.md) for details.
