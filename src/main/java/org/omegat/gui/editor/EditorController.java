...
public class EditorController implements IEditor {
    ...
    synchronized void loadUp(int count) {
        ...
        firstLoaded = loadTo;
        // Re-mark only after every insertion at offset 0 is done, and include
        // the segment that was on top before: a highlight Position at offset 0
        // is pinned by Swing and does not shift with content prepended at 0,
        // so a mark on the very first character of the topmost segment inflates
        // over each insertion until its highlight is removed and set again.
        for (int i = loadTo; i <= oldFirst; i++) {
