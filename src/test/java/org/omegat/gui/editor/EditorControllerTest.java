...
public class EditorControllerTest extends TestCore {
    ...
    public void testLoadUpDoesNotInflateMarkAtDocumentStart() throws Exception {
        ...
        // two more can be prepended. The prepended segments are marked
        // asynchronously by the marker threads; only the segment that was on
        // top before is re-marked synchronously. Late asynchronous deliveries
        // of the load-time passes are harmless: they replace marks
        // idempotently, and results computed against the replaced first
        // document are dropped by the document guard in marksOutput.
        ...
        // The marks of the two prepended segments arrive from the marker
        // threads; wait for their delivery.
        List<Highlighter.Highlight> highlights = awaitFirstCharHighlights(4);
    ...
    private List<Highlighter.Highlight> awaitFirstCharHighlights(int expected) throws Exception {
        long deadline = System.currentTimeMillis() + 5000;
        List<Highlighter.Highlight> highlights = firstCharHighlights();
        while (highlights.size() != expected && System.currentTimeMillis() < deadline) {
            Thread.sleep(20);
            highlights = firstCharHighlights();
        }
        assertEquals(expected, highlights.size());
        return highlights;
    }
